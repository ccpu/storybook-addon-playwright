...
- **`npx` could not resolve the name** — on a version of the config predating the
  ...
  `node /abs/path/to/project/node_modules/storybook-addon-playwright/dist/mcp/cli.js`.
...
The addon's own copy of the bin (`dist/mcp/cli.js`) is still produced by the
