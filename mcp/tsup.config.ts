 ...
 * `dist/mcp/cli.mjs`, so the addon keeps exposing the server as a second bin for
...
export default defineConfig({
  ...
  format: ['esm'],
  // `.mjs` regardless of the package `type`, matching the addon's `dist/mcp/cli.mjs` bin.
  outExtension: () => ({ js: '.mjs' }),
  platform: 'node',
  target: 'node18',
  clean: true,
